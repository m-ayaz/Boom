...
public class Test77 extends Application {
...
//    DataComparator dataComparator = new DataComparator();
    ...
    public void start(Stage stage) throws Exception {
        ...
        for(int  i=0;i<5;i++){
            Rectangle rectangle=new Rectangle(Math.random()*200,Math.random()*200);
            RowConstraints rowConstraints=new RowConstraints();
            rectangle.setFill(new Color(Math.random(),Math.random(),Math.random(),1));
            container.addRow(i,rectangle);
        ...
        for (Node child : container.getChildren()) {
            print(child.getBoundsInParent());
        }

//        print(container.getRowConstraints());

//        container.getCellBounds(0,0);
...
//        x.addListener((ListChangeListener< double[]>) change -> print("changed!"));
//
////        for(int)
//        x.addAll(new double[]{0,0},new double[]{0,0},new double[]{0,0},new double[]{0,0},new double[]{0,0});
//
//        print(x.size());
//        print("asas");
//
