...
//        Random rnd = new Random();
        ...
        Line line=new Line(50,100,200,100);

        container.getChildren().add(line);

        LinearGradient linearGradient=new LinearGradient(0,0,1,1,true, CycleMethod.NO_CYCLE,
                new Stop(0, Color.TRANSPARENT),new Stop(1, Color.rgb(255,0,0,0.5)));
        LinearGradient linearGradient1=new LinearGradient(0,1,0,1,true, CycleMethod.NO_CYCLE,
                new Stop(0, Color.TRANSPARENT),new Stop(1, Color.rgb(0,0,255,0.5)));

//        line.setStrokeWidth(150);

        line.setStyle("-fx-stroke: %s,%s".formatted(
                linearGradient.toString().replace("0x","#"),
                linearGradient1.toString().replace("0x","#")
        ));

        Polyline polyline=new Polyline();
        polyline.getPoints().addAll(50.,50.,200.,200.);
