...
//        getChildren().addAll(addSeries,removeSeries,seriesList);
        ...
        addSeriesButton.setOnAction(event -> {
            ...
            if (appXYChart.type.equals(NodeTypeEnum.AreaChart_NS.getNodeType())) {
            ...
            } else if (appXYChart.type.equals(NodeTypeEnum.LineChart_NS.getNodeType())) {
            ...
            } else if (appXYChart.type.equals(NodeTypeEnum.ScatterChart_NS.getNodeType())) {
        ...
        removeSeriesButton.setOnAction(event -> {
            ...
            if (tab != null) {
                appXYChart.removeSeries(((XYChart) appXYChart.node).getData().indexOf(((SeriesManagementPane_NumberString) ((ScrollPane) tab.getContent()).getContent()).series));
    ...
    public void registerChart(AppXYChart<Number, String> appXYChart) {
        ...
        for (int i = 0; i < ((XYChart) appXYChart.node).getData().size(); i++) {
            XYChart.Series<Number, String> series = ((XYChart<Number, String>) appXYChart.node).getData().get(i);
            Tab tab = new Tab();
